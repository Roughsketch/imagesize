...
#[test]
fn bmp_test_safe() {
    let dim = size_safe("test/test.bmp").unwrap();
    assert_eq!(dim.width, 512);
    assert_eq!(dim.height, 512);
}
...
#[test]
fn gif_test_safe() {
    let dim = size_safe("test/test.gif").unwrap();
    assert_eq!(dim.width, 100);
    assert_eq!(dim.height, 100);
}
...
#[test]
fn jpeg_test_safe() {
    let dim = size_safe("test/test.jpg").unwrap();
    assert_eq!(dim.width, 690);
    assert_eq!(dim.height, 298);
}
...
fn jpeg_extra_info_test_safe() {
    let dim = size_safe("test/extra.jpg").unwrap();
...
#[test]
fn png_test_safe() {
    let dim = size_safe("test/test.png").unwrap();
    assert_eq!(dim.width, 2000);
    assert_eq!(dim.height, 2000);
}
...
#[test]
fn webp_test_safe() {
    let dim = size_safe("test/test.webp").unwrap();
    assert_eq!(dim.width, 716);
    assert_eq!(dim.height, 716);
}
...
fn riffx_webp_test_safe() {
    let dim = size_safe("test/riffx.webp").unwrap();
...
fn blob_test_safe() {
    ...
    let dim = blob_size_safe(&data).unwrap();
    assert_eq!(dim.width, 123);
    assert_eq!(dim.height, 321);
}
