#[cfg(test)]
use imagesize::{
    image_type, ImageType, CompressionFamily, 
    DdsCompression, PvrtcCompression, PkmCompression,
};
...
fn test_compression_family_dds() {
    ...
    assert_eq!(bc1_type.compression_family(), Some(CompressionFamily::BlockCompression));
    
    let bc7_type = ImageType::Dds(DdsCompression::Bc7);
    assert_eq!(bc7_type.compression_family(), Some(CompressionFamily::BlockCompression));
    
    let rgba_type = ImageType::Dds(DdsCompression::Rgba32);
    assert_eq!(rgba_type.compression_family(), Some(CompressionFamily::Uncompressed));
...
fn test_compression_family_pvrtc() {
    ...
    assert_eq!(pvrtc_type.compression_family(), Some(CompressionFamily::Pvrtc));
        ...
        if std::path::Path::new(file_path).exists() {
            ...
            match image_type(&file_data) {
                Ok(img_type) => {
                    ...
                    assert_eq!(img_type.compression_family(), Some(CompressionFamily::BlockCompression));
...
fn test_helper_methods_with_real_pvrtc_data() {
    ...
    let test_cases = [
        ("tests/images/pvrtc/compressions/pvrtc_2bpp_rgb.pvr", CompressionFamily::Pvrtc),
        ("tests/images/pvrtc/compressions/etc2_rgb.pvr", CompressionFamily::Etc),
        ("tests/images/pvrtc/compressions/eac_r.pvr", CompressionFamily::Eac),
    ];
