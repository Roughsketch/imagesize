...
fn test_all_dds_compressions() {
    let test_cases = vec![
        ...
        ("tests/images/dds/compressions/bc6h.dds", DdsCompression::Bc6h),
            ...
            match image_type(&file_data) {
                Ok(ImageType::Dds(compression)) => {
                    assert_eq!(
                        ...
                        "Wrong compression detected for {}",
                        file_path
                    );
                }
                Ok(other) => panic!("Expected DDS format, got {:?} for {}", other, file_path),
                Err(e) => panic!("Failed to detect format for {}: {:?}", file_path, e),
            }
        } else {
            println!("Skipping test for {} (file not found)", file_path);
        }
    }
}
    ...
    for (file_path, expected_compression) in test_cases {
        if std::path::Path::new(file_path).exists() {
            ...
            match image_type(&file_data) {
                ...
                Ok(other) => panic!("Expected ETC2 format, got {:?} for {}", other, file_path),
                Err(e) => panic!("Failed to detect format for {}: {:?}", file_path, e),
            }
        } else {
            println!("Skipping test for {} (file not found)", file_path);
    ...
    for (file_path, expected_compression) in test_cases {
        if std::path::Path::new(file_path).exists() {
            ...
            match image_type(&file_data) {
                ...
                Ok(other) => panic!("Expected EAC format, got {:?} for {}", other, file_path),
                Err(e) => panic!("Failed to detect format for {}: {:?}", file_path, e),
            }
        } else {
            println!("Skipping test for {} (file not found)", file_path);
    ...
    for (file_path, expected_compression) in test_cases {
        if std::path::Path::new(file_path).exists() {
            ...
            match image_type(&file_data) {
                ...
                Ok(other) => panic!("Expected ATC format, got {:?} for {}", other, file_path),
                Err(e) => panic!("Failed to detect format for {}: {:?}", file_path, e),
            }
        } else {
            println!("Skipping test for {} (file not found)", file_path);
    ...
    for (file_path, expected_compression) in test_cases {
        if std::path::Path::new(file_path).exists() {
            ...
            match image_type(&file_data) {
                ...
                Ok(other) => panic!("Expected PVRTC format, got {:?} for {}", other, file_path),
                Err(e) => panic!("Failed to detect format for {}: {:?}", file_path, e),
            }
        } else {
            println!("Skipping test for {} (file not found)", file_path);
